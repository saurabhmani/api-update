...
//  the current price. Signal-only mode serves Yahoo or nothing. // @deprecated marker
// ════════════════════════════════════════════════════════════════

import React from 'react';

export type PriceSource = 'yahoo' | 'none'; // @deprecated marker

const CONFIG: Record<PriceSource, { label: string; dot: string; cls: string }> = {
  yahoo: { label: 'Yahoo (Delayed)', dot: '🟡', cls: 'bg-yellow-50 text-yellow-700 border-yellow-200' }, // @deprecated marker
...
export function PriceSourceBadge({ source }: { source: string | undefined }) {
  const key: PriceSource = source === 'yahoo' ? 'yahoo' : 'none'; // @deprecated marker

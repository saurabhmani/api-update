...
//    - No WebSocket. Yahoo-friendly cadence; no streaming pressure. // @deprecated marker
...
//       leak requests.
//    4. Stale fallback — if Yahoo fails mid-session, we keep showing // @deprecated marker
...
export interface LivePriceTickerProps {
  ...
  symbol: string;
  /** Poll interval in ms. Default 5000. Kept between 3000-10000 to match Yahoo's tolerance. */ // @deprecated marker
  refreshMs?: number;
...
interface PriceApiResponse {
  ...
  pChange?: number | null;
  change?:  number | null;
  source?:  'yahoo' | 'none' | null; // @deprecated marker
  error?:   string;
}
...
export function LivePriceTicker({
  ...
  useEffect(() => {
    ...
    pollTimerRef.current = setInterval(loadPrice, refreshMs);
  ...
  const isDelayed  = source !== 'yahoo' ? false : true;  // Yahoo is always delayed // @deprecated marker

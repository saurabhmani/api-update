...
export default function ConvictionDistribution({ signals, loading, totalScanned }: ConvictionDistributionProps) {
  ...
  return (
    <div style={{
      ...
      // Hard containment: any Recharts tooltip / active-shape SVG that
      // mounts inside this card MUST not leak outside its bounds. The
      // dashboard previously showed ghost numeric labels and dashed
      // lines drifting across other section headers — that's the
      // Recharts tooltip wrapper being absolutely positioned outside
      // the parent because the parent lacked a stacking context AND
      // overflow clipping. `isolation: isolate` creates a fresh
      // stacking context and `contain: layout paint` tells the browser
      // not to paint child overflow outside this box.
      overflow: 'hidden',
      isolation: 'isolate',
      ...
      <div style={{ padding: '16px 20px 20px' }}>
        {total === 0 ? (
          ...
          <div style={{
            ...
            {/* ── Donut Chart ──────────────────────────────────
                The Recharts tooltip wrapper used to escape this 200x200
                box because Recharts portals it with absolute positioning.
                `overflow: hidden` clips it; `isolation: isolate` gives
                it its own stacking context so any leftover absolutely-
                positioned active-shape SVG cannot z-fight other
                dashboard sections; `wrapperStyle` on <Tooltip> caps the
                tooltip width and forbids it from poking past the chart
                bounds. */}
            <div style={{
              position: 'relative',
              width: 200,
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    wrapperStyle={{
                      // Cap the tooltip inside its parent. Recharts ships
                      // a wrapper that absolutely-positions the tooltip;
                      // bounding it here prevents the "ghost numeric
                      // labels over Top Rankings" symptom.
                      pointerEvents: 'none',
                      zIndex: 1,
                      maxWidth: 200,
                    }}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${d.count > 0 ? d.color + '20' : '#E2E8F0'}`,
                      transition: 'all 0.2s',
                      cursor: 'default',
                      // Zero-state used to drop opacity to 0.5 so the
                      // entire row faded — operators kept reading it
                      // as a "loading" or "broken" state. The fix
                      // keeps full opacity so text reads cleanly; the
                      // visual hint that the row is empty is now
                      // carried by muted (but readable) slate text +
                      // a thin bar with a "0" in it. Same idea as
                      // GitHub / Linear stats panels: an explicit "0"
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      background: d.count > 0 ? d.gradient : '#CBD5E1',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      flexShrink: 0,
                    }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          {/* Label readability: zero-state was previously
                              `#94A3B8` (slate-400) which fell below the
                              4.5:1 WCAG contrast bar against the
                              `#FAFBFC` background. Bumped to `#475569`
                              (slate-600 ≈ 7.5:1) so "High Conviction"
                              stays readable when count is 0 — the
                              explicit numeric `0` carries the empty
                              signal, not a fading text style. */}
                          <span style={{
                            fontSize: 12, fontWeight: 700,
                            color: d.count > 0 ? d.textColor : '#475569',
                          }}>
                            {d.label}
                          </span>
                          <span style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                          {/* Numeric value: zero-state was `#CBD5E1`
                              (slate-300, ~2.4:1 contrast) which read
                              as a placeholder rather than data. Now
                              `#475569` so an explicit `0` reads as
                              "zero high-conviction signals" instead
                              of "value not loaded yet." */}
                          <span style={{

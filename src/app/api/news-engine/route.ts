...
export async function GET(req: NextRequest) {
  ...
  // ── Action router (spec aliases: summary, events, source-status) ──
  // Coexist with the legacy boolean-style flags below — `action`
  // wins when both are present.
  const action = sp.get('action');

  // Parse the audit-log row shape once for both summary + source-status.
  // q365_news_ingestion_log columns: run_at DATETIME,
  // source_breakdown_json JSON, total_fetched INT, etc.
  const readLatestRunInfo = async (): Promise<{
    runAt: string | null;
    breakdown: Record<string, number>;
  }> => {
    try {
      const logs = await getRecentIngestionLogs(1);
      const row = logs?.[0] ?? null;
      if (!row) return { runAt: null, breakdown: {} };
      const runAtRaw = row.run_at ?? row.ranAt ?? null;
      const runAt = runAtRaw ? new Date(runAtRaw).toISOString() : null;
      const rawBreakdown = row.source_breakdown_json ?? row.sourceBreakdown ?? {};
      const parsed = typeof rawBreakdown === 'string'
        ? JSON.parse(rawBreakdown || '{}')
        : rawBreakdown;
      return { runAt, breakdown: parsed ?? {} };
    } catch {
      return { runAt: null, breakdown: {} };
    }
  };

  if (action === 'source-status') {
    // No outbound calls — answer from env state only. Augment with
  ...
  if (action === 'summary') {
    ...
    try {
      ...
      // Latest event timestamp comes from the DB so it reflects what
      // the dashboard would actually show, not just the last run.
      let latestNewsAt: string | null = null;
      try {
        const recent = await queryNewsEvents({ limit: 1 });
      ...
      //   FRESH   = ≥1 configured source pulled rows in the last run
      //             AND the latest event is < 1h old
      //   PARTIAL = some configured sources pulled rows but the
      //             freshest event is stale OR several sources failed
      //   STALE   = pipeline ran but no rows came back
      //   NO_DATA = no configured sources OR pipeline has never run
      let status: 'FRESH' | 'PARTIAL' | 'STALE' | 'NO_DATA' = 'NO_DATA';
      if (configured.length === 0 || !last) {
        status = 'NO_DATA';
      } else if (fetchedRecently.length === 0) {
        status = 'STALE';
      } else {
        const ageMs = latestNewsAt
          ? Date.now() - new Date(latestNewsAt).getTime()
          : Number.POSITIVE_INFINITY;
        status = ageMs < 3_600_000 && fetchedRecently.length === configured.length
          ? 'FRESH'
          : 'PARTIAL';
      }
  ...
  // ── Ingestion audit log ────────────────────────────────────
  if (sp.get('logs') === 'true' || action === 'logs') {
    try {
...
export async function POST(req: NextRequest) {
  ...
  // Spec alias — `action: 'run-pipeline'` is a synonym for the default
  // full pipeline path below. Treated identically; surfaced separately
  // so the API contract reads as documented.
  const isExplicitRunPipeline = body.action === 'run-pipeline';
  ...
  // Fallback — when no news was fetched (no API keys configured,
  // adapters offline, etc) we previously seeded synthetic demo events
  // so the UI had something to render. That violates the "do not fake
  // news" policy in production, so the seeder is now gated behind the
  // NEWS_ALLOW_DEMO_SEED env var. Default = disabled. Set it to true
  // in dev/preview environments only.
  const fetchedAnything = (result?.ingestion?.newEvents ?? 0) > 0;
  if (!fetchedAnything && process.env.NEWS_ALLOW_DEMO_SEED === 'true') {
  ...
  return NextResponse.json({
    ok: true,
    action: isExplicitRunPipeline ? 'run-pipeline' : 'pipeline',
    ingestion: result.ingestion,
    scoring: result.scoring,
    impact: impactSummary,
    durationMs: result.durationMs,
    mode: result.mode ?? 'live',
    // Pipeline summary fields per Phase-B spec (also live on
    // `ingestion.*` for legacy callers).
    fetchedTotal:          result.ingestion?.totalFetched ?? 0,
    insertedTotal:         result.ingestion?.newEvents ?? 0,
    duplicateTotal:        result.ingestion?.duplicatesSkipped ?? 0,
    failedSources:         (result.ingestion?.sourceStatus ?? [])
      .filter((s: any) => s.error)
      .map((s: any) => ({ source: s.source, error: s.error })),
    sourceStatus:          result.ingestion?.sourceStatus ?? [],
    latestNewsPublishedAt: result.ingestion?.latestNewsPublishedAt ?? null,
    latestPipelineRunAt:   result.ingestion?.latestPipelineRunAt ?? null,
  }, { status: 201 });
}
  ...
  for (const e of demoEvents) {
    try {
      ...
      await dbConn.query(
        ...
        [
          eventId, e.symbol,
          e.source === 'finnhub' ? 82 : e.source === 'rss_et' ? 68 : e.source === 'rss_mc' ? 64 : 62,

...
const LTP_REFRESH_MS  = 10_000;  // Yahoo batch is cheap; poll at 10s.
const LTP_BATCH_SIZE  = 100;     // Matches yahoo-batch's URL-length cap.
...
export default function MarketPage() {
  ...
  // Stays true until the very first Yahoo LTP batch completes — used
  ...
  /* ── Poll LTPs directly from Yahoo Finance (bulk batch) ── */
  const loadQuotes = useCallback(async (showSpinner = false) => {
    ...
    try {
      ...
      for (let i = 0; i < equityKeys.length; i += LTP_BATCH_SIZE) {
        ...
        try {
          const res = await marketApi.yahooLtp(chunk) as any;
      ...
      // of whether Yahoo returned rows — the UI should not stay locked
      // behind the loader forever if Yahoo is down.
      ...
      <div className="page">
          ...
          <div>
            ...
            <p style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#16A34A', fontWeight: 700 }}>
                <Wifi size={12} /> LIVE · Yahoo Finance
              </span>
        ...
        {initialLoad ? (
          <div style={{ padding: '60px 0' }}>
            <Loading text={listLoad ? 'Loading market universe…' : 'Fetching live prices from Yahoo Finance…'} />
          </div>
        ) : (
          <>
        {indices.length > 0 && (

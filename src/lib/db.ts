...
export function getDb(): mysql.Pool {
  if (!g.__mysqlPool) {
    ...
    // Pool sizing — was 10, raised to 30.
    //
    //   Why this caused 504 Gateway Timeouts on /api/signals:
    //   The dashboard polls /api/signals every ~5s and each request
    //   runs 4–5 queries (signals, breakdowns, freshness probe). The
    //   custom-universe scanner + manipulation worker hold their own
    //   connections during long scans. With only 10 slots, a single
    //   slow query (e.g. the manipulation summary topAlerts hanging
    //   on a missing index) blocked every later request — they queued
    //   forever (queueLimit: 0 = unlimited queue) and nginx eventually
    //   gave up at its 60s read timeout and returned 504.
    //
    //   30 gives breathing room for: ~10 concurrent dashboard polls,
    //   a few worker queries, and a scanner batch — without overwhelming
    //   MySQL (default max_connections is 151, leaving headroom for
    //   other processes).
    //
    //   Configurable via MYSQL_POOL_SIZE if you need to tune further.
    const poolSize = Math.max(
      5,
      Math.min(100, Number(process.env.MYSQL_POOL_SIZE) || 30),
    );
    g.__mysqlPool = mysql.createPool({
      ...
      // instead of piling up until nginx 504s. 100 is generous for the
      // current dashboard polling rate.
      queueLimit:       100,
      namedPlaceholders: false,
      multipleStatements: false,
      // Recycle idle connections after 60s so transient MySQL restarts
      // don't leave us holding dead handles indefinitely.
      idleTimeout:      60_000,

...
export const marketApi = {
  ...
  yahooLtp:(keys: string[]) => // @deprecated marker
    get(`/market?action=yahoo-ltp&keys=${encodeURIComponent(keys.join(','))}`), // @deprecated marker
  quotes:  (keys: string[]) =>
...
// ── Notifications ─────────────────────────────────────────────────
// `id` is now a composite string (`manip-123` / `breach-7` / `notif-12`
// / `sys-weekend-2026-05-02`) emitted by the aggregator at
// /api/notifications. The legacy numeric form is still accepted for
// callers that haven't migrated.
export const notificationsApi = {
  list:    ()                       => get('/notifications'),
  markRead:(id?: string | number)   => post('/notifications', { id }),

...
/** ISO-8601 → MySQL DATETIME (`YYYY-MM-DD HH:MM:SS`). MySQL rejects
 *  the ISO `T`/`Z` form on DATETIME columns; convert at the bind site. */
    ...
    try {
      await db.query(
        ...
        [
          ...
          toMysqlDateTime(l.signalGeneratedAt),
          toMysqlDateTime(l.newsEventPublishedAt),
          l.scoringVersion ?? 'v1',
          toMysqlDateTime(l.linkedAt),
        ],
      );
      saved++;
    } catch (err) {

...
/**
 * MySQL DATETIME columns reject ISO-8601 strings with the `T` separator
 * and `Z` suffix (e.g. `2026-04-25T17:30:00.776Z`). The driver does not
 * coerce — it returns "Incorrect datetime value" and silently drops the
 * row. Convert any ISO string / Date / null into the canonical
 * `YYYY-MM-DD HH:MM:SS` shape MySQL accepts. UTC clock time is
 * preserved (no timezone shift); the column is treated as UTC by
 * convention since the input was UTC.
 */
...
export async function queryNewsScores(filter: ScoreQueryFilter = {}) {
  ...
  if (filter.fromDate) {
    ...
    params.push(toMysqlDateTime(filter.fromDate) ?? filter.fromDate);
  }

...
export interface IndexCardDef {
  ...
  yahoo:   string;   // Yahoo Finance symbol for fallback // @deprecated marker
}

export const INDEX_CARD_DEFS: IndexCardDef[] = [
  { symbol: 'NIFTY 50',      label: 'Nifty 50',    yahoo: '^NSEI'    }, // @deprecated marker
  { symbol: 'NIFTY BANK',    label: 'Bank Nifty',  yahoo: '^NSEBANK' }, // @deprecated marker
  { symbol: 'NIFTY IT',      label: 'Nifty IT',    yahoo: '^CNXIT'   }, // @deprecated marker
  { symbol: 'NIFTY MIDCAP 100', label: 'Midcap',   yahoo: '^NSEMDCP50'}, // @deprecated marker
  { symbol: 'INDIA VIX',     label: 'India VIX',   yahoo: '^INDIAVIX'}, // @deprecated marker
];

// ── Yahoo Finance symbol converter ─────────────────────────────── // @deprecated marker
export function toYahooSymbol(symbol: string): string { // @deprecated marker
  if (!symbol) return '';

  // Already a Yahoo symbol // @deprecated marker

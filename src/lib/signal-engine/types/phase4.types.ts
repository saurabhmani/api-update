...
export interface Phase4SignalEnvelope {
  ...
  // Threaded from ExecutableSignal so saveSignals can persist them
  // to q365_signals.composite_final_score / classification /
  // phase4_factor_scores_json. Mandatory — runPhase4Scoring is the
  // single source of truth for both live and batch outputs.
  final_score:    number;
  ...
  // ── Phase-11 unified row block (optional) ───────────────────
  // Populated by runPhase11Pipeline inside generatePhase4Signals.
  // Null when the pipeline failed for this signal — saveSignals
  // tolerates a null phase11 by writing NULLs to the Phase-11
  // columns, which the read path / Phase-12 partition treat as

...
//    2. Batch-resolve live LTPs via MarketDataResolver (Kite // @deprecated marker
//       primary, Yahoo fallback — same path saveSignals uses, so // @deprecated marker
...
export interface RescoreResult {
  ...
  // These answer the operator's "is Kite actually working?" // @deprecated marker
  // question without requiring a separate health probe. If
  // kiteHits drops to 0 while yahooHits climbs, the Kite // @deprecated marker
  // websocket is stale or disconnected — check loginRequired.
  kiteHits:        number; // @deprecated marker
  yahooHits:       number; // @deprecated marker
...
async function runOnce(): Promise<RescoreResult> {
  ...
  const result: RescoreResult = {
    ...
    kiteHits: 0, yahooHits: 0, otherHits: 0, failedFetches: 0, // @deprecated marker
    staleRescored: 0,
    elapsedMs: 0,
  };
  ...
  for (const q of quotes) {
    if (q.symbol && q.price != null && q.price > 0) {
      ...
      if (src.includes('kite')) result.kiteHits++; // @deprecated marker
      else if (src.includes('yahoo')) result.yahooHits++; // @deprecated marker
  ...
  for (const row of rows) {
    ...
    // When the market is closed, Kite is down, or the provider // @deprecated marker
    ...
    const manipulation = manipulationById.get(row.id) ?? null;
    ...
    const breakdown = computeFinalScore({
      ...
      manipulationPenalty: manipulation?.confidencePenalty ?? null,
      freshness,
      verdict,
    });

    // ── Rejection-engine re-gate: manipulation hard-reject ─────
    // The rescore loop intentionally doesn't re-run runRejectionEngine
    // wholesale — confidence/risk/RR are static at generation and
    // rotation/stop/target/drift are already covered by the
    // postSignalValidator above. The one rejection-engine semantic
    // that DOES need re-checking on every cycle is manipulation:
    // a fresh snapshot may have flagged the symbol for outright
    // rejection since the signal was published. When that happens
    // the rescore force-invalidates the row so it stops surfacing,
    // overriding any softer verdict the post-signal validator
    // produced.
    let finalVerdict = verdict;
    if (manipulation?.shouldReject && verdict.action !== 'invalidate') {
      finalVerdict = {
        action:          'invalidate',
        reason:          'manipulation_rejected' as any,
        nextStatus:      'expired',
        scoreMultiplier: 0,
      };
    }

    if (finalVerdict.action === 'invalidate')      result.invalidated++;
    else if (finalVerdict.action === 'downgrade')  result.downgraded++;

    updates.push({
      id:                 row.id,
      finalScore:         finalVerdict.action === 'invalidate' ? 0 : breakdown.finalScore,
      ...
      invalidationReason: finalVerdict.reason,
      invalidatedAt:      finalVerdict.action === 'invalidate' ? mysqlNow() : null,
      nextStatus:         finalVerdict.nextStatus,
    });
  }
  ...
  const pctKite = result.scanned > 0 // @deprecated marker
    ? Math.round(100 * result.kiteHits / result.scanned) // @deprecated marker
    : 0;
  const feedHealth = pctKite >= 90 ? 'LIVE (Kite)' // @deprecated marker
    : pctKite >= 30 ? 'DEGRADED (mixed)' // @deprecated marker
    : result.yahooHits > 0 ? 'FALLBACK (Yahoo ~15min delay)' // @deprecated marker
    : 'FAIL (no data)';
...
/** Per-signal manipulation snapshot used by the rescore loop. */
interface ManipulationSnapshot {
  /** 0-100 confidence penalty fed into the dynamic ranker. */
  confidencePenalty: number;
  /** True when the manipulation engine has flagged this signal for
   *  outright rejection. The rescore force-invalidates these rows
   *  so a signal that was clean at generation time but tripped a
   *  manipulation check post-publication doesn't sit on the board. */
  shouldReject:      boolean;
}

async function fetchManipulationPenalties(signalIds: number[]): Promise<Map<number, ManipulationSnapshot>> {
  const out = new Map<number, ManipulationSnapshot>();
  if (signalIds.length === 0) return out;
  try {
    const placeholders = signalIds.map(() => '?').join(',');
    const { rows } = await db.query<{ signal_id: string; confidence_penalty: number; rejection_flag: number }>(
      `SELECT signal_id, confidence_penalty, rejection_flag
    ...
    for (const r of rows) {
      ...
      if (isFinite(idNum)) {
        out.set(idNum, {
          confidencePenalty: Number(r.confidence_penalty) || 0,
          shouldReject:      Number(r.rejection_flag) === 1,
        });
      }
    }
  } catch {

...
/** Coerce a non-finite number (NaN / ±Infinity) to NULL before binding
 *  to a SQL parameter. mysql2 serialises a JS NaN as the bare token
 *  `NaN`, which the server then parses as an identifier and rejects
 *  with `ER_BAD_FIELD_ERROR: Unknown column 'NaN' in 'field list'`.
 *  Phase 3 sizing legitimately produces NaN when capital math
 *  degenerates (zero risk, zero quantity, missing inputs); landing
 *  NULL is the honest representation. Verified 2026-05-01 against
 *  the live position-sizing failure that was dropping the artifact
 *  for every Phase 4 run. */
...
async function saveRiskSnapshot(signalId: number, r: Phase3RiskBreakdown): Promise<void> {
  await db.query(
    `INSERT INTO q365_signal_risk_snapshots
     ...
     VALUES (?, ?, ?, ?, ?, ?)`,
    [
      signalId, fin(r.standaloneRiskScore), fin(r.portfolioRiskScore),
      fin(r.totalRiskScore), r.riskBand, JSON.stringify(r.riskFactors),
    ],
  );
}
...
async function saveTradePlan(signalId: number, tp: Phase3TradePlan): Promise<void> {
  await db.query(
    `INSERT INTO q365_signal_trade_plans
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      signalId, tp.entryType, fin(tp.entryZoneLow), fin(tp.entryZoneHigh),
      fin(tp.stopLoss), fin(tp.initialRiskPerUnit),
      fin(tp.target1), fin(tp.target2), fin(tp.target3),
      fin(tp.rrTarget1), fin(tp.rrTarget2), fin(tp.rrTarget3),
    ],
  );
}
...
async function savePositionSizing(signalId: number, s: PositionSizingResult): Promise<void> {
  await db.query(
    `INSERT INTO q365_signal_position_sizing
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      signalId, s.capitalModel, fin(s.portfolioCapital), fin(s.riskBudgetPct),
      fin(s.riskBudgetAmount), fin(s.initialRiskPerUnit), fin(s.positionSizeUnits),
      fin(s.grossPositionValue), s.validationStatus,
...
async function savePortfolioFit(signalId: number, f: PortfolioFitResult): Promise<void> {
  await db.query(
    `INSERT INTO q365_signal_portfolio_fit
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      signalId, fin(f.fitScore), f.sectorExposureImpact, f.directionImpact,
      f.capitalAvailability, f.correlationCluster, fin(f.correlationPenalty),

  ...
  await db.query(
    `INSERT INTO q365_signal_outcomes
      (signal_id, entry_triggered, bars_to_entry,
       ...
       time_to_target_bars, time_to_target2_bars, time_to_target3_bars, time_to_stop_bars,
       holding_duration_bars, bars_unresolved,
       entry_timestamp, resolution_timestamp, signal_generated_at, signal_state_at_resolution,
    ...
    [
      ...
      outcome.timeToTarget2Bars ?? null,
      outcome.timeToTarget3Bars ?? null,
      outcome.timeToStopBars ?? null,
      outcome.holdingDurationBars ?? null,
      outcome.barsUnresolved ?? null,
      outcome.entryTimestamp ? toMysqlDateTime(outcome.entryTimestamp) : null,
      outcome.resolutionTimestamp ? toMysqlDateTime(outcome.resolutionTimestamp) : null,
      outcome.signalGeneratedAt ? toMysqlDateTime(String(outcome.signalGeneratedAt)) : null,
      outcome.signalStateAtResolution ?? null,
...
export async function migratePhase4Tables(): Promise<void> {
  ...
  const outcomeIntelligenceColumns: Array<[string, string]> = [
    ...
    ['time_to_target2_bars', 'INT NULL'],
    ['time_to_target3_bars', 'INT NULL'],
    ['time_to_stop_bars', 'INT NULL'],
    ['holding_duration_bars', 'INT NULL'],
    ['bars_unresolved', 'INT NULL'],
    ['entry_timestamp', 'DATETIME NULL'],
    ['resolution_timestamp', 'DATETIME NULL'],
    ['signal_generated_at', 'DATETIME NULL'],
    ['signal_state_at_resolution', 'VARCHAR(40) NULL'],

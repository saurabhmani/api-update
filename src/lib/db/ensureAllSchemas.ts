...
const ALL_TABLES: string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS q365_manipulation_snapshots (
    ...
    triggered_events_json JSON DEFAULT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  // Weekly NSE 1000 rebuild audit — snapshot + per-symbol churn log.
  ...
  // Durable, per-trading-date maintenance DAG state. Unlike the legacy
  // pipeline lock this records every stage and is therefore also the cheap
  // authoritative source used by health endpoints and missed-run repair.
  ...
  // Strategy Hub — deployment profiles + audit history (Phase 1)
  ...
  // Strategy Hub — mode change audit (Phase 2)
  ...
  // Strategy Hub — validation history (Phase 4)
  ...
  // Strategy Hub — operational monitoring (Phase 6)

...
async function migrate() {
  ...
  try {
    ...
    // Per-user read state for the aggregated alert feed (manipulation
    // events, breaches, system items, future signal/ranking sources).
    // Marking a manipulation event "read" used to mutate the global
    // q365_manipulation_events.status, which hid the row for every user
    // — wrong for personal alerts. The aggregator now reads (source_type,
    // source_id, user_id) from this table and never touches the global
    // manipulation status from the personal mark-read path.
    //
    // source_type values: 'manipulation' | 'breach' | 'notif' | 'sys'
    //                    | 'signal' | 'ranking' | 'watchlist' | 'volatility'
    //                    | 'data_quality'
    // source_id values:  the source row id (integer) or stable composite
    //                    string for synthetic items (e.g. 'weekend-2026-05-02').
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS user_notification_reads (
        ...
        UNIQUE KEY uniq_user_source (user_id, source_type, source_id),
        INDEX idx_unr_user (user_id)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    `);
    console.log('✓ user_notification_reads');

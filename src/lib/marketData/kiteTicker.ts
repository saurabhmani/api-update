// ════════════════════════════════════════════════════════════════
//  kiteTicker — NEUTRALIZED STUB
//
//  The Kite integration has been removed from this system (signal-
//  only analytics mode). This module previously owned the Kite
...
//  data pipeline (Yahoo-primary). Execution-critical callers that
...
export function getLiveTick(_symbol: string): Tick {
  throw new WsDownError('kite_removed');
}
...
class KiteTickerStub extends EventEmitter {
...
const GLOBAL_KEY = '__q365_kite_ticker_stub__';

function getSingleton(): KiteTickerStub {
  const g = globalThis as unknown as Record<string, KiteTickerStub | undefined>;
  if (!g[GLOBAL_KEY]) {
    g[GLOBAL_KEY] = new KiteTickerStub();
  }
  return g[GLOBAL_KEY]!;
}

export function getTicker(): KiteTickerStub {
  return getSingleton();
}

export type { KiteTickerStub as KiteTicker };

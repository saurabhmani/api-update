...
// ════════════════════════════════════════════════════════════════
//  getLivePrice — Kite primary, Yahoo ALWAYS-available fallback // @deprecated marker
//
//  Data flow (STRICT, no activation gates, no policy blocks):
//
//    Layer 1  — Kite in-memory tick cache (O(1)) // @deprecated marker
...
//               (market-closed path).
//
//    Layer 2  — Kite EOD close from market_data_daily VIEW // @deprecated marker
//               Projected candles from the Kite ingest pipeline. // @deprecated marker
//               Useful off-hours when the WS isn't producing ticks
//               but we still have an authoritative last close.
//
//    Layer 3  — Yahoo Finance (UNCONDITIONAL fallback) // @deprecated marker
//               Always tried if Layer 1+2 missed. No loginRequired
//               gate, no "market-open disables Yahoo" policy, no // @deprecated marker
//               wait-for-poller-to-activate dance. Yahoo answers or // @deprecated marker
...
import { getTicker, isFresh } from './kiteTicker'; // @deprecated marker
...
import {
  isYahooAvailable, // @deprecated marker
  recordYahooSuccess, // @deprecated marker
  recordYahooFailure, // @deprecated marker
} from './yahooCircuitBreaker'; // @deprecated marker

const log = logger.child({ component: 'getLivePrice' });

export type PriceSource = 'kite' | 'yahoo' | 'none'; // @deprecated marker
...
export type PriceResponse = {
  ...
  ageMs?:  number;
  source:  PriceSource;
  /** True when the tick is from Kite but older than the live-freshness // @deprecated marker
...
// Freshness threshold for "Kite is active" on a specific symbol. // @deprecated marker
...
// Max age of a cached Kite tick or EOD bar before we consider it // @deprecated marker
// "too old to show" and fall through to Yahoo. Default: 3 days // @deprecated marker
...
// 15-min-delayed Yahoo snapshot over a years-old Kite close. // @deprecated marker
const STALE_KITE_MS =
 ...
 */
export function isKiteActive(symbol: string): boolean { // @deprecated marker
 ...
 *
 *   1. Kite in-memory cache (any age when market closed; ≤ MAX_KITE_AGE_MS // @deprecated marker
 *      when market open). This is a fully synchronous O(1) read off the
 *      ticker's `ticksBySymbol` Map — only populated by real Kite WS // @deprecated marker
 *      frames, never by Yahoo. // @deprecated marker
 *   2. Kite EOD close from `market_data_daily` VIEW (Kite-sourced candles // @deprecated marker
...
export async function getLivePrice(symbol: string): Promise<PriceResponse> {
  ...
  // downstream layer (Kite cache, EOD query, Yahoo) expects the // @deprecated marker
  // bare form; ad-hoc trim()/toUpperCase() lived in multiple places
  // before and let "NSE:RELIANCE" slip through to Kite EOD lookups // @deprecated marker
  ...
  // ── Layer 1: Kite in-memory cache ─────────────────────────────── // @deprecated marker
  // getTickBySymbolSync reads the ticker's Kite-only Map directly // @deprecated marker
  // (see kiteTicker.ts — ticksBySymbol is populated ONLY by the // @deprecated marker
  // 'ticks' bridge in the KiteTicker constructor). No age gate // @deprecated marker
  // here — a stale Kite tick is the authoritative last-traded // @deprecated marker
  ...
  if (tick && tick.lastPrice != null && tick.lastPrice > 0) {
    ...
    //                  fall through to Yahoo for a fresher snapshot. // @deprecated marker
      ...
      if (Number.isFinite(close) && close > 0) {
        ...
        if (age > STALE_KITE_MS) {
          console.log(
            ...
            `age=${Math.round(age / 86400000)}d → falling to Yahoo`, // @deprecated marker
          );
        } else {
          console.log(
          ...
          return {
            ...
            ageMs:   age,
            source:  'kite', // Kite-sourced EOD from candles // @deprecated marker
            stale:   true,
          };
        }
      }
  ...
  // ── Layer 3: Yahoo (UNCONDITIONAL fallback, circuit-breaker gated) ── // @deprecated marker
  //
  // Any symbol that missed Kite cache AND Kite EOD gets a direct // @deprecated marker
  // Yahoo lookup. No activation flag, no loginRequired check, no // @deprecated marker
  // market-open policy. If Yahoo answers, the caller gets a price. // @deprecated marker
  //
  // The ONE protective gate: yahooCircuitBreaker. After 20 consecutive // @deprecated marker
  // Yahoo failures the breaker trips OPEN for 5 minutes — callers // @deprecated marker
  // short-circuit to "no data" during the pause. This prevents a
  // 2700-symbol rescore cycle from spamming Yahoo into an IP ban // @deprecated marker
  // when the upstream is rate-limiting us or down.
  //
  // withProviderFrame is required because yahoo.ts has a tripwire // @deprecated marker
  // (enforcer) that rejects unframed callers in staging/prod.
  if (!isYahooAvailable()) { // @deprecated marker
    console.log(`[YAHOO FAIL] ${sym} breaker=OPEN (paused to prevent IP ban)`);
    return { price: null, source: 'none', error: 'yahoo_breaker_open' }; // @deprecated marker
  }

  try {
    const yahoo = await withProviderFrame(() => fetchFromYahoo(sym)); // @deprecated marker
    if (yahoo && yahoo.price != null && yahoo.price > 0) { // @deprecated marker
      recordYahooSuccess(); // @deprecated marker
      console.log(`[YAHOO SUCCESS] ${sym} price=${yahoo.price}`); // @deprecated marker
      log.debug('Yahoo served', { symbol: sym, price: yahoo.price }); // @deprecated marker
      return {
    ...
    // Yahoo just doesn't know this ticker. Count it as a failure // @deprecated marker
    // so a systematic Yahoo outage trips the breaker, but the // @deprecated marker
    // individual symbol returns cleanly.
    recordYahooFailure('null_or_zero_price'); // @deprecated marker
    console.log(`[YAHOO FAIL] ${sym} (null/zero price)`);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    recordYahooFailure(message); // @deprecated marker
    console.log(`[YAHOO FAIL] ${sym} error=${message}`);
    log.warn('Yahoo fetch failed', { symbol: sym, error: message }); // @deprecated marker
  }

...
//  ticker itself (kiteTicker.ts) already holds a Map<symbol,Tick> // @deprecated marker
...
import { getTicker } from './kiteTicker'; // @deprecated marker
...
class TickStore {
  // Authoritative Kite cache — only populated by frames with // @deprecated marker
  // source='kite' (or absent source, which the ticker uses for real // @deprecated marker
  // WS frames). This is the source of truth for "last traded price"
  // and deliberately never decays: a Friday-close Kite LTP must // @deprecated marker
  // still be readable on Saturday so the UI can render it.
  private readonly kiteStore  = new Map<string, TickData>(); // @deprecated marker
  // Yahoo shadow cache — used only for symbols that have no Kite // @deprecated marker
  // entry yet. A Kite tick for the same symbol takes over // @deprecated marker
  // immediately and the Yahoo entry becomes dormant. // @deprecated marker
  private readonly yahooStore = new Map<string, TickData>(); // @deprecated marker
  ...
  install(): void {
    ...
    // Seed the Kite cache from the ticker's current snapshot (the // @deprecated marker
    // ticker only stores Kite frames, so this never pollutes). // @deprecated marker
    const seed = getTicker().getAllTicks();
    for (const t of seed) {
      if (t.symbol) this.kiteStore.set(t.symbol.toUpperCase(), t); // @deprecated marker
    }
    console.log(`[tickStore] installed  seeded=${seed.length}  (kite-only)`); // @deprecated marker
  }
  ...
  /** O(1) lookup with source priority — Kite entry wins, Yahoo is // @deprecated marker
   *  only returned when Kite has never produced a tick for this // @deprecated marker
   *  symbol. Returns null if neither cache has the symbol. */
  get(symbol: string): TickData | null {
    assertNotBacktest('get');
    const key = symbol.trim().toUpperCase();
    return this.kiteStore.get(key) ?? this.yahooStore.get(key) ?? null; // @deprecated marker
  }

  /** Explicit Kite-only read. Callers that MUST not accept a Yahoo // @deprecated marker
   *  fallback (e.g. execution paths) should use this. */
  getKite(symbol: string): TickData | null { // @deprecated marker
    assertNotBacktest('getKite'); // @deprecated marker
    return this.kiteStore.get(symbol.trim().toUpperCase()) ?? null; // @deprecated marker
  }

  has(symbol: string): boolean {
    assertNotBacktest('has');
    const key = symbol.trim().toUpperCase();
    return this.kiteStore.has(key) || this.yahooStore.has(key); // @deprecated marker
  }
  ...
  size(): number {
    ...
    // misleading.
    const union = new Set<string>(this.kiteStore.keys()); // @deprecated marker
    for (const k of this.yahooStore.keys()) union.add(k); // @deprecated marker
    return union.size;
  }

  /**
   * Materialise the merged view (Kite wins). O(n). For debug // @deprecated marker
   * endpoints only — do NOT call from a tick handler or an API
   * hot path.
   */
  snapshot(): TickData[] {
    const merged = new Map<string, TickData>();
    for (const [k, t] of this.yahooStore) merged.set(k, t); // @deprecated marker
    for (const [k, t] of this.kiteStore)  merged.set(k, t); // @deprecated marker

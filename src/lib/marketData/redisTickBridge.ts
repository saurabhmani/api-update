...
//  them back out WITHOUT touching the Kite WebSocket directly.
//
//  Layout
//  ──────
...
export function installRedisTickBridge(): void {
  ...
  const onTick = (t: TickData): void => {
    ...
    // Source gate — Kite-first, Yahoo only when no Kite entry exists
    // for this symbol. Without this, a Yahoo poll cycle overwrites
    // the authoritative Kite LTP in `tick:latest:<SYM>` and every
    // downstream Redis reader sees Yahoo data. The separate hash
    // `tick:latest:yahoo:<SYM>` is still written so operators can
    // inspect the shadow channel without polluting truth.
    const isYahoo = t.source === 'yahoo';
    ...
    if (isYahoo) {
      ...
      pipe.hset(`${LATEST_PREFIX}yahoo:${sym}`, fields);
      // Only SET the authoritative key if NO Kite entry exists.
      ...
      // Kite frame — authoritative write, always wins.

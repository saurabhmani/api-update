...
const rateLog = logger.child({ component: 'indianApiRateLimiter' });
  ...
  async acquire(group: IndianApiEndpointGroup): Promise<void> {
    ...
    try {
      ...
      for (;;) {
        ...
        if (pause > 0) {
          rateLog.info('IndianAPI rate limit wait', {
            event: 'indianapi_rate_limit_wait',
            reason: 'global_pause',
            group,
            waitMs: pause,
        ...
        if (await this.tryConsume(group)) {
          ...
          if (waitedMs > 50) {
            rateLog.info('IndianAPI rate limit wait', {
              ...
              group,
              waitMs: waitedMs,
              rpsGlobal: getIndianApiIngestConfig().rpsGlobal,
              stats: this.stats(),
            });
          }
          return;
        ...
        const windowWaitMs = nextWindow - nowMs + Math.floor(Math.random() * 100);
        rateLog.info('IndianAPI rate limit wait', {
          event: 'indianapi_rate_limit_wait',
          reason: 'rps_window',
          group,

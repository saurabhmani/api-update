...
export async function migrateManipulationEngineTables(): Promise<void> {
  // ── Self-heal: detect a broken q365_manipulation_events schema and drop it.
  //
  // CREATE TABLE IF NOT EXISTS silently skips when the table already
  // exists, even if its columns don't match what the current code needs.
  // We've seen production DBs with an old/partial version of the table
  // missing core columns like `symbol` and `event_date`, which makes
  // every read fail with ER_BAD_FIELD_ERROR. Detect that case here and
  // drop the broken tables so the CREATEs below rebuild them cleanly.
  // Safe: rows in a schema-broken table are unreadable by the app today,
  // so dropping doesn't lose anything the system can use.
  try {
    ...
    if (cols.length > 0) {
      const present = new Set(cols.map((r) => String((r as any).COLUMN_NAME).toLowerCase()));
      const required = ['symbol', 'event_date', 'event_type', 'severity', 'score'];
      const missing = required.filter((c) => !present.has(c));
      if (missing.length > 0) {
        console.warn(
          `[migrate] q365_manipulation_events missing columns [${missing.join(', ')}] ` +
          `— dropping broken manipulation tables to rebuild from scratch.`,
        );
        await db.query('SET FOREIGN_KEY_CHECKS = 0');
        await db.query('DROP TABLE IF EXISTS q365_manipulation_calibration_snapshots');
        await db.query('DROP TABLE IF EXISTS q365_manipulation_watchlist_history');
        await db.query('DROP TABLE IF EXISTS q365_manipulation_watchlists');
        ...
        console.warn('[migrate] dropped broken manipulation tables — recreating now.');
      }
    }
  } catch (probeErr) {
    console.warn(
      '[migrate] schema-drift probe failed (continuing with CREATE IF NOT EXISTS):',

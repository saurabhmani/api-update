...
import { runNightlyBacktestForTradingDate } from './nightlyBacktestStage';
import { repairActiveSignalRiskGeometry } from './riskGeometryRepair';
...
export { repairActiveSignalRiskGeometry } from './riskGeometryRepair';
...
export function createProductionMaintenanceStages(): MaintenanceStage[] {
  return [
    ...
    { name: 'nightly_backtest', dependencies: ['backtesting_evaluation'], critical: true, maxAttempts: 2,
      async run({ tradingDate }) {
      const result = await runNightlyBacktestForTradingDate(tradingDate);
      return { status: 'succeeded', counts: { processed: 1, succeeded: 1 },
        metadata: { runId: result.runId, signalCount: result.signalCount, tradeCount: result.tradeCount } };
    } },
    { name: 'learning_review', dependencies: ['nightly_backtest'], maxAttempts: 2, async run() {

...
export type LivePrice = {
  ...
  change:  number | null;
  pChange: number | null;
  /** Previous-day close from Kite's ohlc.close. Used by the UI to // @deprecated marker
 ...
 *   - source          ('kite' | 'yahoo' | 'none' from the server) // @deprecated marker
 *   - last frame age  (lastAt from the WS — proves data IS flowing)
 *
 * States:
 *   'KITE_LIVE'       Market open, real-time Kite ticks in last 10s // @deprecated marker
 *   'YAHOO_FALLBACK'  Market open, Yahoo poll is carrying the feed // @deprecated marker
...
export interface UseLivePricesResult {
  ...
  source:      'kite' | 'yahoo' | 'none' | null; // @deprecated marker
...
// In signal-only mode the Kite stream server (port 3001) is not booted // @deprecated marker
// — see server.js:53. Without this kill-switch the browser keeps
// retrying ws://localhost:3001 forever and logs a fresh
// "WebSocket connection failed" error on every backoff tick. Set
// NEXT_PUBLIC_STREAM_WS_DISABLED=true in .env.local to short-circuit
// the connect path; the hook still returns a stable shape (empty
// prices, connected=false) and consumers fall back to the SSE stream
// + Yahoo enrichment they already use when the WS is silent. // @deprecated marker
const WS_DISABLED =
  (process.env.NEXT_PUBLIC_STREAM_WS_DISABLED ?? '').toLowerCase() === 'true';
...
// stale threshold, so "Kite went silent" is reflected in the UI // @deprecated marker
...
export function useLivePrices(): UseLivePricesResult {
  ...
  const [source, setSource] = useState<'kite' | 'yahoo' | 'none' | null>(null); // @deprecated marker
  ...
  // pull it on a 5s interval so the UI can tell "Kite silent because // @deprecated marker
  // market closed" from "Kite silent because something broke" — and // @deprecated marker
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (j.source === 'kite' || j.source === 'yahoo' || j.source === 'none') { // @deprecated marker
          setSource(j.source);
        }
  ...
  // Priority ladder (highest wins) — Kite-first policy: // @deprecated marker
  //   1. WS not connected                → DISCONNECTED
  //   2. Server reports source='yahoo'   → YAHOO_FALLBACK  (UI label: "Delayed") // @deprecated marker
  //      NOTE: Yahoo can only appear after market close (when Kite // @deprecated marker
  //      cache + EOD both missed). It must beat MARKET_CLOSED so
  //      the UI shows "Delayed" instead of "Last Close", because
  //      the data is NOT Kite-sourced in this narrow case. // @deprecated marker
  //   3. Market closed                   → MARKET_CLOSED   (UI label: "Last Close")
  //      Data is source='kite' from cached tick OR EOD VIEW. // @deprecated marker

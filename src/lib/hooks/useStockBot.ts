...
//           live Kite ticks reflect in the UI within the flush
...
export function useStockBot(symbols: string[]): UseStockBotResult {
  ...
  // Live WS overlay — merged on top so real Kite ticks flip
  ...
  // Merge live WS frames on top of the 1-Hz baseline. A Kite frame
  ...
  for (const [sym, wsFrame] of wsPrices) {
    ...
    // source='kite' when Kite has observed the symbol.

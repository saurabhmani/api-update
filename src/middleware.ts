...
export function middleware(req: NextRequest) {
  ...
  // Per-request path log was useful for debugging session-routing issues
  // but fired on every API call (and the dashboard polls /api/signals
  // every few seconds). Gated behind LOG_VERBOSE_MIDDLEWARE=1 so it
  // can be turned back on selectively without flooding PM2 log files.

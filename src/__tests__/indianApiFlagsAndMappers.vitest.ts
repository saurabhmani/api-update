...
describe('providerFlags — indianapi resolution', () => {
  it('explicit MARKET_DATA_PROVIDER always wins over the bootstrap default', () => {
    process.env.MARKET_DATA_PROVIDER = 'kite';
    process.env.INDIANAPI_ENABLED = 'true';
    process.env.INDIANAPI_API_KEY = 'k';
    expect(getSystemMarketDataProvider()).toBe('kite');
...
describe('validateEnv — indianapi credential gates', () => {
  ...
  it('bootstrap flag on without key → warning about resolving to none', () => {
    process.env.INDIANAPI_ENABLED = 'true';
    const result = validateEnv();
    expect(result.warnings.some(w => w.includes('resolves to none'))).toBe(true);
  });
});
...
describe('indianApiMappers', () => {
  ...
  it('mapHistorical extracts the price dataset, sorts ascending, drops junk', () => {
    const series = mapHistorical('INFY', '1mo', {
      datasets: [
        { metric: 'Volume', values: [['2026-07-01', 999]] },
        {
          metric: 'Price',
          values: [
            ['2026-07-02', '1,510.5'],
    ...
    // Close-only series: OHLC collapse to close, volume 0.
    expect(series.candles[0].o).toBe(series.candles[0].c);
    expect(series.candles[0].v).toBe(0);
  });

...
// Self-heal flag: ensures we only attempt the CREATE TABLE once per
// process even if many requests race in before the table is ready.
let _decisionTracesEnsured = false;

async function ensureDecisionTracesTable(): Promise<void> {
  if (_decisionTracesEnsured) return;
  // Mirrors src/lib/db/migrateCanonical.ts so the dashboard works even
  // when the canonical migration has never been run on this database.
  // Idempotent — IF NOT EXISTS makes repeat boots a no-op.
  await db.query(`
...
export async function listDecisionTraces(opts?: {
  ...
  // Self-heal so the dashboard's Recent Decisions card never 500s when
  // the canonical migration hasn't been run. Previously this surfaced
  // as ER_NO_SUCH_TABLE → /api/decisions/traces returning an error
  // envelope → the UI silently rendering an empty list with no hint
  // about what was wrong.
  try {
    await ensureDecisionTracesTable();
  } catch (mErr) {
    console.warn('[listDecisionTraces] table-ensure failed (continuing):', (mErr as any)?.message);
  }
  ...
  // Wrap both queries — if the table still doesn't exist after the
  // ensure attempt (e.g. permissions issue), return empty rather than
  // 500 the whole endpoint. Empty list is the right UX for "no
  // decisions yet" and matches the empty-state already in the UI.
  try {
    ...
    console.warn('[listDecisionTraces] query failed — returning empty:', (err as any)?.message);

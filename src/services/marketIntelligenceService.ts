...
// ── Build a MarketSnapshot from a live Kite WebSocket tick ──────
...
function snapshotFromTick(sym: string, t: TickData): MarketSnapshot {
  ...
  //   1. Kite-provided pChange (available in quote/full mode)
  //   2. Previous session close (`t.close` in Kite's wire format)
...
// Priority:
//   1. In-process tickStore   — populated by Kite WebSocket. Fastest,
...
export async function computeMarketIntelligence(): Promise<MarketIntelligenceResult> {
  ...
  //    Skip if cached result has no movers — forces a fresh live fetch
  ...
  // atomic source (Kite → market stocks → indices fallback).
  ...
  // Derive trend score from unified breadth
  const totalBreadthStocks = advancing + declining + unchanged;
  if (totalBreadthStocks > 0) {

...
//    1. Kite WebSocket tickStore (in-process, sub-second)
...
export interface MarketDataset {
  ...
  source: 'kite' | 'market_stocks' | 'market_indices';
  timestamp: string;
  stockCount: number;
...
// ── Layer 1: Kite WebSocket tickStore ───────────────────────────

function tryKiteTicks(): StockEntry[] | null {
  try {
...
export async function fetchUnifiedMarketData(): Promise<MarketDataset> {
  ...
  // ── Layer 1: Kite WebSocket ticks (fastest, freshest) ────────
  const kiteTicks = tryKiteTicks();
  if (kiteTicks) {
    dataset = deriveFromStocks(kiteTicks, 'kite');
    log.info('Market data from Kite tickStore', { stocks: dataset.stockCount });
  }
